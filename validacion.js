...
function valida(input) {
    ...
    if (input.validity.valid) {
        if(input.value.length > 300) {
            input.nextElementSibling.innerHTML = "El campo mensaje debe contener un máximo de 300 caracteres"
  ...
  const mensajesDeError = {
    ...
    asunto: {
      ...
      patternMismatch:
      "El campo asunto debe contener un máximo de 50 caracteres",
    },
    mensaje: {
...
function checkInputsCorrectos(){
    let correctos = Object.values(valid).every(
        value => value === true
    )
    if (correctos === true) {
